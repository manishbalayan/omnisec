...
impl NetworkEnforcementEngine {
    ...
    /// Create a new engine with DB-recovered lists instead of hardcoded defaults.
    /// If `allow_list` or `block_list` is empty, the hardcoded defaults are used as fallback.
    pub fn with_lists(allow_list: Vec<String>, block_list: Vec<String>) -> Self {
        Self {
            allow_list: if allow_list.is_empty() {
                Self::default_allow_list()
            } else {
                allow_list.into_iter().collect()
            },
            block_list: if block_list.is_empty() {
                Self::default_block_list()
            } else {
                block_list.into_iter().collect()
            },
            temp_blocks: Vec::new(),
            actions: Vec::new(),
        }
...
impl ProcessEnforcementEngine {
    ...
    /// Create a new engine with DB-recovered lists instead of hardcoded defaults.
    /// If `known` or `blocked` is empty, the hardcoded defaults are used as fallback.
    pub fn with_lists(known: Vec<String>, blocked: Vec<String>) -> Self {
        Self {
            known_executables: if known.is_empty() {
                Self::default_known_executables()
            } else {
                known.into_iter().collect()
            },
            blocked_executables: if blocked.is_empty() {
                Self::default_blocked_executables()
            } else {
    ...
    pub fn get_known_executables(&self) -> Vec<String> {
        let mut list: Vec<String> = self.known_executables.iter().cloned().collect();
        list.sort();
        list
    }

    pub fn get_blocked_executables(&self) -> Vec<String> {
        let mut list: Vec<String> = self.blocked_executables.iter().cloned().collect();
        list.sort();
        list
    }
...
impl FileAccessEnforcementEngine {
    ...
    /// Create a new engine with DB-recovered sensitive paths instead of hardcoded defaults.
    /// If `paths` is empty, the hardcoded defaults are used as fallback.
    pub fn with_lists(paths: Vec<String>) -> Self {
        Self {
            sensitive_paths: if paths.is_empty() {
                Self::default_sensitive_paths()
            } else {
                paths
            },
            violations: Vec::new(),
    ...
    pub fn get_sensitive_paths(&self) -> Vec<String> {
...
impl EnforcementManager {
    ...
    /// Create an enforcement manager from DB-recovered lists.
    /// Falls back to hardcoded defaults for any empty list.
    pub fn with_lists(
        allow_destinations: Vec<String>,
        block_destinations: Vec<String>,
        known_executables: Vec<String>,
        blocked_executables: Vec<String>,
        sensitive_paths: Vec<String>,
    ) -> Self {
        Self {
            network: NetworkEnforcementEngine::with_lists(allow_destinations, block_destinations),
            process: ProcessEnforcementEngine::with_lists(known_executables, blocked_executables),
            file_access: FileAccessEnforcementEngine::with_lists(sensitive_paths),
            incidents: Vec::new(),
        }
    }
