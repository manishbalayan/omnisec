...
async fn main() -> Result<()> {
        ...
        state_manager
    {
        match sm.recover_all().await {
            Ok(snapshot) => {
                ...
                let enforce_count = snapshot.enforcement_lists.len();
                let restart_count = snapshot.restart_state.len();
                let recovery_count = snapshot.recovery_actions.len();
                if id_count > 0 || enforce_count > 0 || restart_count > 0 || recovery_count > 0 {
                    tracing::info!(
                        "State recovery complete: {} identities, {} enforcement items, {} restart entries, {} pending recoveries restored",
                        id_count, enforce_count, restart_count, recovery_count,
                    );
                } else {
                    tracing::info!("State recovery: no prior state found (fresh start)");
                }
                Some(Arc::new(snapshot))
            }
            Err(e) => {
                tracing::warn!("State recovery skipped (non-fatal): {}", e);
                None
            }
        }
    } else {
        tracing::info!("State recovery: database not available, running in memory-only mode");
        None
    };

    // --- Shared state for health endpoint and production metrics ---
            ...
            for event in &events {
                match event {
                    ...
                    HealthEvent::AgentRecovered { pid, name } => {
                        ...
                        {
                            let mut s = state_for_health.write().await;
                            s.recovery_count += 1;
    ...
    let restart_snapshot = recovered_snapshot.clone();
    tokio::spawn(async move {
        ...
        // Hydrate restart engine with recovered state
        if let Some(ref snap) = restart_snapshot {
            let imported = snap.restart_state.len();
            for entry in &snap.restart_state {
                restart_engine.register_agent(entry.pid, entry.agent_name.clone());
                for _ in 0..entry.attempt_count {
                    restart_engine.record_attempt(entry.pid);
                }
            }
            if imported > 0 {
                tracing::info!(
                    ...
                    for (pid, name) in &pending {
                        ...
                        match new_pid {
                            Some(new_pid) => {
                                ...
                                // Metrics: count successful restart
                                {
                                    let mut s = state_restart.write().await;
                                    s.total_restarts += 1;
                                    s.successful_restarts += 1;
                                    let total = s.successful_restarts + s.failed_restarts;
                                    if total > 0 {
                                        s.restart_success_rate = (s.successful_restarts as f64 / total as f64) * 100.0;
                                    }
                                }
                            }
                            None => {
    ...
    // =====================================================================
    {
        ...
        let state_sec = daemon_state.clone();
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                // --- Metrics: update event throughput (agents processed per second) ---
                {
                    let mut s = state_sec.write().await;
                    // Events per second in this 5-second window
                    s.event_throughput = pids.len() as f64 / 5.0;
                }
    ...
    // =====================================================================
    {
        ...
        let enforce_snapshot = recovered_snapshot.clone();
        let state_enforce = daemon_state.clone();

        tokio::spawn(async move {
            let mut decision_engine = DecisionEngine::new();

            // Use DB-recovered enforcement lists; fall back to hardcoded defaults if empty
            let mut enforcement_manager = if let Some(ref snap) = enforce_snapshot {
                EnforcementManager::with_lists(
                    snap.enforcement_lists.allow_destinations.clone(),
                    snap.enforcement_lists.block_destinations.clone(),
                    snap.enforcement_lists.known_executables.clone(),
                    snap.enforcement_lists.blocked_executables.clone(),
                    snap.enforcement_lists.sensitive_paths.clone(),
                )
            } else {
                EnforcementManager::new()
            };
            ...
            loop {
                ...
                tokio::select! {
                    ...
                    Some((_subject, envelope)) = anomaly_sub.next() => {
                        let _event_start = std::time::Instant::now();
                        ...
                        // Metrics: update detection latency (time from event arrival to decision)
                        {
                            let elapsed_ms = _event_start.elapsed().as_millis() as f64;
                            let mut s = state_enforce.write().await;
                            // Rolling average with 10-event window
                            s.detection_latency_ms = s.detection_latency_ms * 0.9 + elapsed_ms * 0.1;
                        }
    ...
    // =====================================================================
    {
        ...
        let kernel_snapshot = recovered_snapshot.clone();
        let state_kernel = daemon_state.clone();
        ...
        tokio::spawn(async move {
            ...
            // Create identity engine for PID→Agent resolution, hydrated from DB
            let mut identity_engine_obj = AgentIdentityEngine::new();
            ...
            loop {
                tokio::select! {
                    ...
                    // Periodic: log stats and update nats_throughput metric
                    _ = tokio::time::sleep(Duration::from_secs(30)) => {
                        ...
                        // Metrics: NATS throughput ≈ kernel events per second (delta since last cycle)
                        {
                            use std::sync::atomic::{AtomicU64, Ordering};
                            static LAST_EVENTS: AtomicU64 = AtomicU64::new(0);
                            let current = stats.events_total;
                            let last = LAST_EVENTS.swap(current, Ordering::Relaxed);
                            let delta = current.saturating_sub(last);
                            let mut s = state_kernel.write().await;
                            s.nats_throughput = delta as f64 / 30.0;
                        }
                    }
                }
            }
    ...
    let health_app = Router::new()
        ...
        .route(
            "/metrics",
            axum::routing::get(move || daemon_metrics_handler(daemon_state.clone())),
        );
    let health_bind =
...
// =====================================================================
// Sprint 5: Prometheus Metrics Endpoint (daemon-level observability)
// =====================================================================

/// Daemon-level Prometheus metrics endpoint — wired on port 9090 by default.
/// Serves: detection latency, event throughput, NATS throughput,
/// restart success rate, recovery time.
pub async fn daemon_metrics_handler(
    state: Arc<RwLock<DaemonState>>,
) -> impl axum::response::IntoResponse {
    let st = state.read().await;
    let mut out = String::new();

    out.push_str("# HELP omnisec_agent_total Total discovered agents\n");
    out.push_str("# TYPE omnisec_agent_total gauge\n");
    out.push_str(&format!("omnisec_agent_total {}\n\n", st.agent_count));

    out.push_str("# HELP omnisec_agent_alive Agents currently alive\n");
    out.push_str("# TYPE omnisec_agent_alive gauge\n");
    out.push_str(&format!("omnisec_agent_alive {}\n\n", st.alive_count));

    out.push_str("# HELP omnisec_agent_failed Failed agents\n");
    out.push_str("# TYPE omnisec_agent_failed gauge\n");
    out.push_str(&format!("omnisec_agent_failed {}\n\n", st.failed_count));

    out.push_str("# HELP omnisec_restarts_total Total restart attempts\n");
    out.push_str("# TYPE omnisec_restarts_total counter\n");
