...
async fn main() {
    ...
    results.push(check_ebpf());
    results.push(check_redis_ping().await);
...
// ── Sprint 3: eBPF checks ──────────────────────────────────────────────────

fn check_ebpf() -> CheckResult {
    #[cfg(target_os = "linux")]
    {
        // Check for BTF support (required for CO-RE eBPF)
        let btf_supported = std::path::Path::new("/sys/kernel/btf/vmlinux").exists();

        // Check for eBPF syscall availability
        let ebpf_available = std::path::Path::new("/sys/fs/bpf").exists() || btf_supported;

        // Check capabilities via /proc/self/status
        let cap_bpf = std::fs::read_to_string("/proc/self/status")
            .ok()
            .and_then(|s| {
                s.lines()
                    .find(|l| l.starts_with("CapEff:"))
                    .and_then(|l| l.split_whitespace().nth(1))
                    .and_then(|s| u64::from_str_radix(s, 16).ok())
                    .map(|eff| eff & (1 << 39) != 0) // CAP_BPF = bit 39
            })
            .unwrap_or(false);

        let cap_perfmon = std::fs::read_to_string("/proc/self/status")
            .ok()
            .and_then(|s| {
                s.lines()
                    .find(|l| l.starts_with("CapEff:"))
                    .and_then(|l| l.split_whitespace().nth(1))
                    .and_then(|s| u64::from_str_radix(s, 16).ok())
                    .map(|eff| eff & (1 << 38) != 0) // CAP_PERFMON = bit 38
            })
            .unwrap_or(false);

        if btf_supported && (cap_bpf || cap_perfmon) {
            CheckResult::pass(
                "ebpf",
                "BTF vmlinux present, CAP_BPF/CAP_PERFMON available — eBPF sensors ready",
            )
        } else if btf_supported {
            CheckResult::warn(
                "ebpf",
                "BTF present but missing CAP_BPF/CAP_PERFMON — eBPF will fall back to /proc",
                "Add AmbientCapabilities=CAP_BPF CAP_PERFMON to systemd unit or use --privileged in Docker",
            )
        } else if ebpf_available {
            CheckResult::warn(
                "ebpf",
                "eBPF sysfs available but no BTF vmlinux — requires CONFIG_DEBUG_INFO_BTF=y kernel",
                "Rebuild kernel with CONFIG_DEBUG_INFO_BTF=y or use a distro kernel 5.4+",
            )
        } else {
            CheckResult::warn(
                "ebpf",
                "eBPF not available — /proc fallback mode will be used",
                "Run on Linux kernel 5.4+ with CONFIG_DEBUG_INFO_BTF=y",
            )
        }
    }
    ...
    CheckResult::warn(
        "ebpf",
        "not Linux — eBPF unavailable (simulated mode active)",
        "Run Omnisec on Linux kernel 5.4+ with eBPF support",
    )
}

/// Sprint 3: Redis PING via redis protocol (not just TCP probe)
async fn check_redis_ping() -> CheckResult {
    let url = std::env::var("REDIS_URL")
        .unwrap_or_else(|_| "redis://localhost:6379".to_string());
    let addr = extract_host_port(&url, 6379);

    // Try to do a real PING via TCP
            ...
            match stream.read(&mut buf) {
                Ok(n) if n > 0 => {
                    ...
                    if response.contains("+PONG") {
                        CheckResult::pass("redis-ping", &format!("PONG from {}", addr))
                    } else {
                        CheckResult::warn("redis-ping", &format!("unexpected response: {}", response.trim()), "Check Redis configuration")
                    }
                }
                _ => CheckResult::warn("redis-ping", "connected but no PONG response", "Check Redis configuration"),
            }
        }
        Err(e) => CheckResult::fail("redis-ping", &format!("cannot connect: {}", e), "docker compose up -d redis"),
    }
}

/// Sprint 3: DNS resolution check
fn check_dns_resolution() -> CheckResult {
    use std::net::ToSocketAddrs;
    let test_domains = ["api.openai.com", "api.anthropic.com", "github.com"];
    let mut resolved = 0;
    ...
    if resolved == test_domains.len() {
        CheckResult::pass("dns", &format!("{}/{} known domains resolve", resolved, test_domains.len()))
    } else if resolved > 0 {
        CheckResult::warn(
            "dns",
            &format!("{}/{} domains resolve; failed: {}", resolved, test_domains.len(), failed.join(", ")),
            "Check DNS configuration in /etc/resolv.conf",
        )
    } else {
        CheckResult::fail(
            "dns",
            "no known AI API domains resolve — DNS may be broken",
