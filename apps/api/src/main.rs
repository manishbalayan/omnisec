...
struct AppState {
    storage: Storage,
    state_manager: Option<omnisec_state::StateManager>,
...
// =====================================================================
// Dynamic Enforcement List Management (Sprint 2)
// =====================================================================

async fn get_dynamic_list(
    State(state): State<Arc<RwLock<AppState>>>,
...
async fn add_to_dynamic_list(
    ...
    // Persist to database
    {
        ...
        if let Some(ref sm) = st.state_manager {
            let db_type = match list_type.as_str() {
                "block" => Some("block"),
                "allow" => Some("allow"),
                "known-exec" => Some("known_exec"),
                "blocked-exec" => Some("blocked_exec"),
                _ => None,
            };
            if let Some(dt) = db_type {
                let _ = sm.add_enforcement_entry(dt, &target, &reason, "api").await;
            }
        }
    }

    Json(json!({"status": "added", "target": target, "list_type": list_type}))
}

async fn remove_from_dynamic_list(
    State(state): State<Arc<RwLock<AppState>>>,
    Path(list_type): Path<String>,
    Json(body): Json<Value>,
) -> Json<Value> {
...

    {
        ...
        if let Some(ref sm) = st.state_manager {
            let db_type = match list_type.as_str() {
                "block" => "block",
                "allow" => "allow",
                "known-exec" => "known_exec",
                "blocked-exec" => "blocked_exec",
                _ => "",
            };
            if !db_type.is_empty() {
                let _ = sm.remove_enforcement_entry(db_type, &target).await;
            }
        }
    }

    Json(json!({"status": "removed", "target": target}))
}

async fn sync_dynamic_list(
    State(state): State<Arc<RwLock<AppState>>>,
    Path(list_type): Path<String>,
    Json(body): Json<Value>,
) -> Json<Value> {
    ...
    if let Some(sm) = state.state_manager.as_ref() {
        ...
        let _ = sm.persist_enforcement_lists(db_type, &entries).await;
        Json(json!({"status": "synced", "list_type": list_type, "count": entries.len()}))
    } else {
...
async fn get_all_config(State(state): State<Arc<RwLock<AppState>>>) -> Json<Value> {
    ...
    if let Some(ref sm) = state.state_manager {
        match sm.get_all_config().await {
            Ok(config) => Json(json!({"config": config})),
            Err(e) => Json(json!({"error": e.to_string()})),
        }
    } else {
        Json(json!({"config": {}, "note": "Database not available"}))
    }
}
...
async fn get_config(
    ...
    let state = state.read().await;
    if let Some(ref sm) = state.state_manager {
        match sm.get_config(&key).await {
            Ok(Some(value)) => Json(json!({"key": key, "value": value})),
